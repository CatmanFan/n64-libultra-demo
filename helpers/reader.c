...
void read_segment(char *offset, char *start, char *end)
{
	// Static
...
void read_binary(void *dst, u32 offset, u32 size)
{
	u32 readSize;
	u8 *dst_dram = (u8 *)dst;
	...
	osWritebackDCache(dst_dram, size);
	osInvalDCache(dst_dram, size);
	
	while (size)
	{
		// Cap to block size if needed
		u32 max_size = 16384;
		readSize = size > max_size ? max_size : size;

		// Determine parameters of where binary is stored and size
		msgIo_dma.dramAddr = dst_dram;
		msgIo_dma.devAddr  = offset;
		msgIo_dma.size     = readSize;

		// Start reading from ROM and wait for message
		osEPiStartDma(rom_handle, &msgIo_dma, OS_READ);
		osRecvMesg(&msgQ_dma, NULL, OS_MESG_BLOCK);

		// Increment for next ROM read until we run out of size value
		offset += readSize;
		dst_dram += readSize;
		size -= readSize;
	}
